  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<{
        types: { type: NotificationChannelType; name: string; configHints: Record<string, string> }[];
...
export function useCreateNotificationChannel() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      const response = await api.post<{ channel: NotificationChannel }>('/admin/notifications', data);
