		...
		try {
			await updateMutation.mutateAsync({
				...
				data: {
					...
					nextActionDue: editForm.nextActionDue
						? new Date(editForm.nextActionDue).toISOString()
						: null,
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-4">
					...
					<div>
						...
						{contact.role && <p className="text-dark-400 mt-1">{contact.role}</p>}
					</div>
				</div>
			...
			>
				<div className="space-y-4">
					<div className="grid grid-cols-2 gap-4">
						<Select
							...
							onChange={(e) =>
								setInteractionForm({
									...interactionForm,
									type: e.target.value as ContactInteractionType,
								})
							}
						/>
						<Select
							...
							onChange={(e) =>
								setInteractionForm({
									...interactionForm,
									direction: e.target.value as InteractionDirection,
								})
							}
						/>
					</div>
					...
					<Input
						...
						onChange={(e) => setInteractionForm({ ...interactionForm, url: e.target.value })}
...
function getWarmthBadgeVariant(
	...
	return (
		(
			{ cold: "info", warm: "warning", hot: "danger" } as Record<
				string,
				"info" | "warning" | "danger"
			>
		)[warmth] || "default"
