...
export const companyContractTypeEnum = ["b2b", "employment", "both", "unknown"] as const;
...
export const contentEngagementTypeEnum = ["like", "comment", "repost", "share"] as const;
 ...
 */
export const notificationChannelTypeEnum = ["discord", "telegram", "email"] as const;
export type NotificationChannelType = (typeof notificationChannelTypeEnum)[number];
...
export const notificationChannels = pgTable("notification_channels", {
	...
	// Events to notify on
	events: text("events").array().notNull().default(sql`'{"lead.created"}'::text[]`),
...
export const contacts = pgTable(
	"contacts",
	{
		...
		relationshipStatus: varchar("relationship_status", { length: 50 })
			.notNull()
			.default("identified"),
		warmth: varchar("warmth", { length: 20 }).notNull().default("cold"),
	...
	(table) => [
		...
		check("valid_warmth", sql`${table.warmth} IN ('cold', 'warm', 'hot')`),
		check("valid_tier", sql`${table.tier} IS NULL OR ${table.tier} IN ('A', 'B', 'C')`),
	],
);
...
export const contactInteractions = pgTable(
	...
	(table) => [
		...
		check("valid_direction", sql`${table.direction} IN ('inbound', 'outbound')`),
	],
);
