...
import { getLeadUrl, isDiscordConfig } from "./types.js";
 ...
 * @returns Discord webhook payload
 */
export function formatDiscordPayload(payload: NotificationPayload): DiscordWebhookPayload {
...
export function validateDiscordConfig(config: unknown): {
	...
	if (!DISCORD_WEBHOOK_REGEX.test(cfg.webhook_url)) {
		return {
			valid: false,
			error: "Invalid Discord webhook URL. Must be https://discord.com/api/webhooks/{id}/{token}",
		};
	}
