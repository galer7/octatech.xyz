...
describe("shouldRefreshSession", () => {
	...
	it("should return true when just under threshold", () => {
		...
		const expiresAt = new Date(
			new Date("2024-01-01T12:00:00Z").getTime() + SESSION_CONFIG.refreshThresholdMs - 1,
		);
...
describe("validateSession", () => {
	...
	it("should return session data for valid session", async () => {
		...
		const mockLimit = vi.fn().mockResolvedValue([{ session: mockSession, user: mockUser }]);
		const mockWhere = vi.fn().mockReturnValue({ limit: mockLimit });
		const mockInnerJoin = vi.fn().mockReturnValue({ where: mockWhere });
		const mockFrom = vi.fn().mockReturnValue({ innerJoin: mockInnerJoin });
		(db.select as ReturnType<typeof vi.fn>).mockReturnValue({
...
describe("refreshSession", () => {
	...
	it("should refresh session with remember me duration", async () => {
		const newExpiresAt = new Date(Date.now() + SESSION_CONFIG.rememberMeDurationMs);
		const mockReturning = vi.fn().mockResolvedValue([{ expiresAt: newExpiresAt }]);
		const mockWhere = vi.fn().mockReturnValue({ returning: mockReturning });
		const mockSet = vi.fn().mockReturnValue({ where: mockWhere });
		(db.update as ReturnType<typeof vi.fn>).mockReturnValue({
...
describe("deleteUserSessions", () => {
	...
	it("should delete all sessions for a user", async () => {
		const mockReturning = vi
			.fn()
			.mockResolvedValue([{ id: "session-1" }, { id: "session-2" }, { id: "session-3" }]);
		const mockWhere = vi.fn().mockReturnValue({ returning: mockReturning });
		(db.delete as ReturnType<typeof vi.fn>).mockReturnValue({
			where: mockWhere,
		});
	...
	it("should delete all except specified session when exceptSessionId is provided", async () => {
		const mockReturning = vi.fn().mockResolvedValue([{ id: "session-1" }, { id: "session-2" }]);
		const mockWhere = vi.fn().mockReturnValue({ returning: mockReturning });
		(db.delete as ReturnType<typeof vi.fn>).mockReturnValue({
			where: mockWhere,
		});
...
describe("cleanupExpiredSessions", () => {
	...
	it("should delete expired sessions", async () => {
		const mockReturning = vi.fn().mockResolvedValue([{ id: "expired-1" }, { id: "expired-2" }]);
		const mockWhere = vi.fn().mockReturnValue({ returning: mockReturning });
		(db.delete as ReturnType<typeof vi.fn>).mockReturnValue({
			where: mockWhere,
		});
