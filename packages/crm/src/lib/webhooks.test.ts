...
vi.mock("dns/promises", () => ({
	lookup: vi.fn(() => Promise.resolve([{ address: "1.2.3.4", family: 4 }])),
}));
...
describe("formatLeadStatusChangedPayload", () => {
	...
	it("should handle all status transitions", () => {
		...
		for (let i = 0; i < statuses.length - 1; i++) {
			...
			const payload = formatLeadStatusChangedPayload(lead, statuses[i], statuses[i + 1]);
			const data = payload.data as LeadStatusChangedData;
...
describe("formatLeadDeletedPayload", () => {
	it("should return a valid webhook payload structure", () => {
		const payload = formatLeadDeletedPayload("lead-123", "John Doe", "john@example.com");
	...
	it("should include leadId, name, and email in data", () => {
		const payload = formatLeadDeletedPayload("lead-456", "Jane Smith", "jane@example.com");
...
describe("scheduleRetry", () => {
	...
	it("should schedule retry with correct delay from RETRY_DELAYS_MS", () => {
		...
		expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`${RETRY_DELAYS_MS[1]}ms`));
	});
	...
	it("should not schedule retry when max attempts exceeded", () => {
		...
		expect(console.log).toHaveBeenCalledWith(expect.stringContaining("exceeded max retries"));
	});
	...
	it("should log retry scheduling information", () => {
		...
		expect(console.log).toHaveBeenCalledWith(expect.stringContaining(webhook.id));
	});
});
