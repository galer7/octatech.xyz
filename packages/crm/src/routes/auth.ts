 ...
 */
authRoutes.post("/change-password", requireAuth, requireCsrfHeader, async (c) => {
	const session = requireSession(c);
	...
	const [user] = await db.select().from(adminUser).where(eq(adminUser.id, session.userId)).limit(1);
