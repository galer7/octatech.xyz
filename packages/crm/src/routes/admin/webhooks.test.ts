...
describe("Admin Webhooks Routes", () => {
  ...
  describe("GET /api/admin/webhooks/:id/deliveries", () => {
    it("should return 401 when not authenticated", async () => {
      ...
      const res = await app.request(
        "/api/admin/webhooks/webhook_123/deliveries"
      );
    ...
    it("should respect page and limit parameters", async () => {
      ...
      const res = await app.request(
        "/api/admin/webhooks/webhook_123/deliveries?page=2&limit=10",
        {
          headers: authHeaders(),
        }
      );
    ...
    it("should return 404 for non-existent webhook", async () => {
      ...
      const res = await app.request(
        "/api/admin/webhooks/nonexistent/deliveries",
        {
          headers: authHeaders(),
        }
      );
    ...
    it("should format delivery dates as ISO strings", async () => {
      ...
      const mockDeliveries = [
        createMockDelivery({ id: "delivery_1", attemptedAt }),
      ];
    ...
    it("should cap limit at 100", async () => {
      ...
      const res = await app.request(
        "/api/admin/webhooks/webhook_123/deliveries?limit=200",
        {
          headers: authHeaders(),
        }
      );
  ...
  describe("GET /api/admin/webhooks/events/list", () => {
    it("should return all valid events with descriptions", async () => {
      ...
      const eventNames = body.events.map(
    ...
    it("should return specific descriptions for known events", async () => {
      ...
      const body = await res.json();
      const leadCreated = body.events.find(
        (e: { event: string }) => e.event === "lead.created"
      );
