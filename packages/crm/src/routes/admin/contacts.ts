...
import { isOpenAIConfigured, parseContactText } from "../../lib/ai/index.js";
...
adminContactsRoutes.get("/", async (c) => {
	...
	return c.json({
		data: contactsResult.map((row) => ({
			...
			company: row.companyJoinId ? { id: row.companyJoinId, name: row.companyName } : null,
		})),
		pagination: {
			page,
			limit,
...
adminContactsRoutes.patch("/:id", async (c) => {
	...
	if (input.relationshipStatus && input.relationshipStatus !== existingContact.relationshipStatus) {
...
adminContactsRoutes.delete("/:id", async (c) => {
	...
	if (contact.leadId) {
		const leadsTable = await import("../../db/index.js").then((m) => m.leads);
		await db.update(leadsTable).set({ contactId: null }).where(eq(leadsTable.id, contact.leadId));
	}
...
adminContactsRoutes.post("/parse", async (c) => {
	...
	if (!isOpenAIConfigured()) {
		throw new BadRequestError("AI parsing not available. OpenAI API key not configured.");
	}
