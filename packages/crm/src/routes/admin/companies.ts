...
import { type Company, type Contact, companies, contacts, db } from "../../db/index.js";
...
adminCompaniesRoutes.get("/", async (c) => {
	...
	if (search) {
		...
		conditions.push(
			or(ilike(companies.name, searchPattern), ilike(companies.industry, searchPattern)),
		);
	}
	...
	// Get total count
	const countQuery = db.select({ count: sql<number>`count(*)::int` }).from(companies);
	...
	const contactCountSq =
		sql<number>`(SELECT count(*)::int FROM contacts WHERE contacts.company_id = ${companies.id})`.as(
			"contactCount",
		);
