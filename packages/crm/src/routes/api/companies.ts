...
import { type Company, type Contact, companies, contacts } from "../../db/schema.js";
...
companiesApiRoutes.get("/", requireScope("companies:read"), async (c) => {
	...
	let companiesQuery = db.select().from(companies).orderBy(orderBy).limit(limit).offset(offset);
	...
	if (companyIds.length > 0) {
		const contactCountRows = await db
			...
			.from(contacts)
			.where(sql`${contacts.companyId} IN ${companyIds}`)
