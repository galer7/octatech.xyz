 ...
 */
function createValidLeadPayload(
 ...
 */
function setupDbMock(options: {
  leadId?: string;
  shouldFail?: boolean;
  errorMessage?: string;
} = {}) {
  const { leadId = "test-lead-id-123", shouldFail = false, errorMessage = "Database error" } = options;

  // Track all mock calls for verification
  const allValuesCalls: unknown[] = [];
  const allReturningCalls: unknown[] = [];
  ...
  describe("POST /api/leads", () => {
    ...
    describe("Honeypot spam protection", () => {
      ...
      it("should log the spam attempt when honeypot is filled", async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
          expect.stringContaining("bot@spammer.com")
        );
      });
