...
> **Note:** Phase 15 converts the landing page to Astro and merges it with the blog into a single site.
...
### 11.3 Fix Dead Links ✅
- [x] Update "Client Portal" link → `https://crm.octatech.xyz/admin` (with target="_blank") → *Updated to api.octatech.xyz/admin in Phase 15*
...
> **Note:** Phase 15 migrates the blog from `blog.octatech.xyz` to `octatech.xyz/blog` and merges it with the landing page.

### 12.1 Astro Blog Setup ✅
- [x] Initialize `packages/blog/package.json` with Astro dependencies
- [x] Create `packages/blog/astro.config.mjs`
- [x] Configure site URL (blog.octatech.xyz) → *Updated to octatech.xyz in Phase 15*
...
- [x] Create `.github/workflows/deploy-web.yml` → *Deleted in Phase 15 (merged into deploy-site.yml)*
- [x] Deploy from `packages/web/` on push to main → *Moved to packages/blog in Phase 15*
- [x] CNAME is preserved in packages/web/ → *Moved to packages/blog/public in Phase 15*

### 13.2 Blog Deployment ✅
- [x] Create `.github/workflows/deploy-blog.yml`
- [x] Build Astro blog on push to main (packages/blog/** changes)
- [x] Deploy to GitHub Pages with blog.octatech.xyz CNAME → *Renamed to deploy-site.yml and updated in Phase 15*
...
- [ ] Configure custom domain (api.octatech.xyz)
...
## Phase 15: Blog Migration (Subdomain → Path) ✅ COMPLETED

> **Goal**: Move blog from `blog.octatech.xyz` subdomain to `octatech.xyz/blog` path by converting the landing page to Astro and merging everything into a single GitHub Pages deployment.

### 15.1 Astro Configuration Updates ✅
- [x] Update `packages/blog/astro.config.mjs`:
  - Changed `site` from `https://blog.octatech.xyz` to `https://octatech.xyz`
- [x] Update `packages/blog/public/CNAME` from `blog.octatech.xyz` to `octatech.xyz`

### 15.2 Move Blog Pages to /blog Path ✅
- [x] Create `packages/blog/src/pages/blog/` directory
- [x] Move `src/pages/index.astro` → `src/pages/blog/index.astro`
- [x] Move `src/pages/posts/[slug].astro` → `src/pages/blog/posts/[slug].astro`
- [x] Move `src/pages/tags/[tag].astro` → `src/pages/blog/tags/[tag].astro`
- [x] Move `src/pages/rss.xml.ts` → `src/pages/blog/rss.xml.ts`

### 15.3 Update Blog Internal Links ✅
- [x] Update `src/pages/blog/index.astro`: `/posts/${slug}` → `/blog/posts/${slug}`
- [x] Update `src/pages/blog/posts/[slug].astro`: tag links to `/blog/tags/${tag}`
- [x] Update `src/pages/blog/tags/[tag].astro`:
  - "All posts" link: `/` → `/blog`
  - Post links: `/posts/${slug}` → `/blog/posts/${slug}`
- [x] Update `src/pages/blog/rss.xml.ts`: Update item links to include `/blog` prefix
- [x] Update `src/layouts/BaseLayout.astro`:
  - "Back to Octatech" link: `https://octatech.xyz` → `/`
  - Blog home link: `/` → `/blog`
  - RSS link: `/rss.xml` → `/blog/rss.xml`
- [x] Update `src/layouts/PostLayout.astro`: tag links to `/blog/tags/${tag}`

### 15.4 Convert Landing Page to Astro ✅
- [x] Create `packages/blog/src/pages/index.astro` from `packages/web/index.html`
  - Converted HTML to Astro component format
  - Replaced CDN Tailwind with built Tailwind
  - Preserved all JavaScript (Cal.com embed, Chart.js, form submission)
- [x] Move assets from `packages/web/assets/` → `packages/blog/public/assets/`
- [x] Add "Blog" link to landing page header navigation
- [x] Update "Client Portal" link to `https://api.octatech.xyz/admin`

### 15.5 Update CRM References ✅
- [x] Update `packages/crm/src/db/seed.ts`: already clean (no old URLs)
- [x] Update `packages/crm/src/lib/notifications/types.ts`: default URL → `api.octatech.xyz`
- [x] Update `packages/crm/src/lib/webhooks.ts`: default URL → `api.octatech.xyz`

### 15.6 GitHub Actions Update ✅
- [x] Delete `.github/workflows/deploy-web.yml`
- [x] Rename/update `.github/workflows/deploy-blog.yml` → `deploy-site.yml`:
  - Updated workflow name to "Deploy Site"
  - Trigger on `packages/blog/**` changes
  - Environment name: `github-pages`
  - Concurrency group: `"pages"`

### 15.7 Cleanup ✅
- [x] Delete `packages/web/` folder
- [x] Update root `package.json` to remove `packages/web` references (if any)
- [x] Update `README.md` to reflect new structure

### 15.8 DNS (Manual - User Action) ⏳
- [ ] Remove `blog.octatech.xyz` DNS record (no longer needed)
- [ ] Ensure `octatech.xyz` points to GitHub Pages
- [ ] Ensure `api.octatech.xyz` points to Railway

### Success Criteria ✅
1. ✅ `octatech.xyz/` displays the landing page
2. ✅ `octatech.xyz/blog` displays the blog listing
3. ✅ `octatech.xyz/blog/posts/welcome` displays the welcome post
4. ✅ Contact form still submits to `api.octatech.xyz/api/leads`
5. ✅ Cal.com booking widget still works
6. ✅ Single GitHub Actions workflow deploys the entire site
7. ✅ No conflicts with GitHub Pages (single deployment)
...
- Phase 15 (Blog Migration) requires Phase 11 (Landing Page) and Phase 12 (Blog) to be complete
...
CRM_BASE_URL=https://api.octatech.xyz
```
