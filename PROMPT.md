1.  **Context:** Study `specs/*` to learn requirements and `IMPLEMENTATION_PLAN.md` for the current task list.

2.  **Task:** Follow the `IMPLEMENTATION_PLAN.md` and choose the **most important single item**. Implement this functionality using parallel subagents.
...
5.  **Clean Up:**
    * When a task is done and tests pass, update `IMPLEMENTATION_PLAN.md` to remove the item using a subagent.
...
999. **CRITICAL INSTRUCTIONS:**
    ...
    * Always keep `IMPLEMENTATION_PLAN.md` up to date with your learnings.